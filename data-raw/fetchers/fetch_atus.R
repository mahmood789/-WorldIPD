# Fetch ATUS (American Time Use Survey) microdata using a mapping CSV
# mapping CSV columns: year,file,url
# Requires: readr

fetch_atus <- function(map_file = 'data-raw/fetchers/atus_url_map.csv', dest_dir = file.path('inst','extdata')){
  ...
  if (inherits(mp,'try-error') || !all(c('year','file','url') %in% names(mp))) stop('Invalid ATUS map CSV; expected year,file,url')
  for (i in seq_len(nrow(mp))) {
    yr <- mp$year[i]; lab <- mp$file[i]; url <- mp$url[i]
    id <- sprintf('atus_%s_%s', yr, lab)
